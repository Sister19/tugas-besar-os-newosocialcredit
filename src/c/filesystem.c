...
void readSector(byte* buffer, int sector_number){
    ...
    int cylinder = (sector_number / SECTORS_PER_CYLINDER) << 8;
    int sector = mod(sector_number, SECTORS_PER_HEAD) + 1;
    int head = mod((sector_number / SECTORS_PER_HEAD), 2) << 8;
    int drive = 0x00;

    // call interrupt
    interrupt(
        0x13, // interrupt 0x13 untuk read/write sectors
        0x0201, // 0x0201 untuk read
        buffer, // buffer
        cylinder + sector, //CH untuk cylinder, CL untuk sektor
        head + drive // DH untuk head, DL untuk drive
    );
}
